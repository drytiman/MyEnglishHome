...
public class TestServer {
    ...
    void test_server(){
        try (CloseableHttpClient httpclient = HttpClientBuilder.create().build()){
            ...
            StudentDto studentDto = OBJECT_MAPPER.readValue(jsonStudent, StudentDto.class);
            Assertions.assertEquals("name", studentDto.getName());

            JavaType studentList = OBJECT_MAPPER.getTypeFactory().constructCollectionType(List.class, StudentDto.class);

            InputStream jsonStudents = getContentFromUri(API_ADDRESS + "/students", httpclient);
            List<StudentDto> users = OBJECT_MAPPER.readValue(jsonStudents, studentList);
